    ...
    class PostsController < ApplicationController
      ...
      def show_by_reducing_attempt
        ...
        if @attempts
          ...
          render json: {msg: "you dont have subscription"}
        end
      ...
      def update
        ...
        # reading_time = (params[:content].split.size / 60.0).ceil
      ...
      def destroy
        @revision = Revision.new(
          action: "Deleted",
          post_id: @post.id, # Assuming there's a post with ID 1
          user_id: @current_user.id, # Assuming there's a user with ID 1
        )
        if @revision.save
          if @post.destroy
